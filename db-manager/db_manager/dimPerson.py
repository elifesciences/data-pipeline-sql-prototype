...
def registerInitialisations(conn, source, column_map):
    ...
    if ('status' not in column_map):
        column_map['status'] = """'Unknown(' || {externalReference_Person} || ')'""".format(
            **column_map
        )

    if ('first_name' not in column_map):
        column_map['first_name'] = """'Unknown(' || {externalReference_Person} || ')'""".format(
            **column_map
        )

    if ('middle_name' not in column_map):
        column_map['middle_name'] = """'Unknown(' || {externalReference_Person} || ')'""".format(
            **column_map
        )

    if ('last_name' not in column_map):
        column_map['last_name'] = """'Unknown(' || {externalReference_Person} || ')'""".format(
            **column_map
        )
    ...
    DBManager.registerInitialisations(
        ...
        source=source,
        allowed_columns=['source_file_name', 'source_file_creation', 'externalReference_Person',
                         'status', 'first_name', 'middle_name', 'last_name', 'profile_modify_date'],
